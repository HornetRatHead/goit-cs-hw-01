...
class TokenType:
    ...
    #множення MUL, ділення DIV та дужок, які відкривають LPAREN та закривають RPAREN
...
class Parser:
    ...
    def factor(self):
        ...
        if token.type in (TokenType.PLUS, TokenType.MINUS):
            self.eat(token.type)
            node = self.factor()  # Обробляємо фактор

            # Якщо унарний оператор -, створюємо BinOp, що виконує 0 - фактор
            if token.type == TokenType.MINUS:
                node = BinOp(left=Num(Token(TokenType.INTEGER, 0)), op=token, right=node)
            return node
            ...
            node = self.expr()  # Обробляємо вираз у дужках
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            raise Exception(f"Операція не підтримується: {node.op.type}")
